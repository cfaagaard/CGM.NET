...
namespace CGM.Win.HidDevice.Win32
{
    ...
    public abstract class HIDDevice : Win32Usb, IDisposable
    {
        ...
        private void Initialise(string strPath)
        {
            ...
            _handle = CreateFile(strPath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE | FILE_SHARE_READ, IntPtr.Zero, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, IntPtr.Zero);
            if (_handle != InvalidHandleValue)	// if the open worked...
            {
                ...
                if (HidD_GetPreparsedData(_handle, out lpData))	// get windows to read the device data into an internal buffer
                {
                    try
                    {
                        ...
                        ProductString = HidD_GetProductString(_handle, buffer, 126) ? Marshal.PtrToStringAnsi(buffer) : string.Empty;
        ...
        private void BeginAsyncRead()
        {
            ...
            //_file.ReadAsync(arrInputReport, 0, InputReportLength);
        ...
        protected virtual void Write(OutputReport oOutRep)
        {
            try
            {
        ...
        public static oType FindDevice<oType>(int nVid, int nPid) where oType : HIDDevice
        {
            ...
            try
            {
                ...
                for (var i = 0; SetupDiEnumDeviceInterfaces(hInfoSet, 0, ref gHid, (uint)i, ref oInterface); i++)	// this gets the device interface information for a device at index 'i' in the memory block
                {
                    ...
                    oType oNewDevice = (oType)Activator.CreateInstance(typeof(oType));	// create an instance of the class for this device
        ...
        public static oType FindDeviceByPath<oType>(string devicePath) where oType : HIDDevice
        {
            oType oNewDevice = (oType)Activator.CreateInstance(typeof(oType));
            oNewDevice.Initialise(devicePath);
            return oNewDevice;
        }

        public static List<oType> FindDevices<oType>(int nVid) where oType : HIDDevice
        {
            var searchPath = GetSearchPath(nVid);
            return FindDevices<oType>(searchPath);
        }

        public static List<oType> FindDevices<oType>(int nVid, int nPid) where oType : HIDDevice
        {
            var searchPath = GetSearchPath(nVid, nPid);
            return FindDevices<oType>(searchPath);
        }

        public static List<oType> FindDevices<oType>(string searchPath) where oType : HIDDevice
        {
            ...
            try
            {
                ...
                for (var i = 0; SetupDiEnumDeviceInterfaces(hInfoSet, 0, ref gHid, (uint)i, ref oInterface); i++)	// this gets the device interface information for a device at index 'nIndex' in the memory block
                {
                    ...
                    var oNewDevice = (oType)Activator.CreateInstance(typeof(oType));	// create an instance of the class for this device
                    try
                    {
                        oNewDevice.Initialise(strDevicePath);	// initialise it with the device path
                        list.Add(oNewDevice);
        ...
        public static string GetSearchPath(int nVid)
        {
            return string.Format("vid_{0:x4}", nVid);
        }
